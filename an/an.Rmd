...
rm(blocks, cur_block, cur_subj, cur_ds, cur_subj_dir)
head(ds)
```
...
# Number of confidence == 0
ds_conf = ds
ds_conf$Contrast = paste(ds$stim1_c, ds$stim2_c, sep='\n')
ds_conf = ddply(ds_conf, .(subj, resp_int_conf, Contrast), summarise, 
                count=length(resp_int_conf))
p = ggplot(data=ds_conf, aes(x=Contrast, y=count, fill=factor(resp_int_conf))) + 
  geom_bar(stat='identity') + labs(fill='Confidence')
print(p)
```
